...
const Customer = () => {
    ...
    const updateDatabase = async (x, rID) => {
        const data1 = await axios.patch(`http://localhost:5000/api/customers/${rID}`, {x: x, add: true})
        const data2 = await axios.patch(`http://localhost:5000/api/customers/${id}`, {x: x, add: false})
